...
## 執行 Checkpoint 1I Rule Interpretation Foundation

Checkpoint 1I 只讀 frozen Checkpoint 1H outputs 與本機版本化 [`knowledge/pokemon/rules/v1/`](knowledge/pokemon/rules/v1/)。Interpretation 是獨立衍生紀錄：只能引用 Battle Fact IDs，不建立或修改 Battle Facts，也不把 knowledge inference 冒充 observation。

```bash
.venv/bin/pokemon-battle-vision checkpoint-1i \
  --project-root . \
  --checkpoint-1h-dir outputs/checkpoint-1h \
  --output outputs/checkpoint-1i
```
