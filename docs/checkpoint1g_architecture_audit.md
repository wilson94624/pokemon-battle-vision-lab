...
| SELECTED_FOUR | 1 個 candidate；新 onboarding 使用完整六列 player roster ROI | 逐列保存六列 evidence，只以可見 1–4 marker 決定任意四列的 selection order；列可不連續，marker 不足時保留 unknown | 是 |
...
Team Selection 畫面固定顯示六隻 roster，但可選的是其中任意四隻；selected rows 不保證連續，1–4 marker 也可能出現在六列中的任何位置。因此 `selected_four` 必須涵蓋完整 player roster，不能以「下方四列」或 crop 順序代替 selection order。Team Preview 與 Team Selection 可共用幾何，ROI ID、purpose 與 parser responsibility 仍維持分離。
